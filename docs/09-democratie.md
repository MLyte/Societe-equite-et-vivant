---
status: draft
version: "0.1"
last_updated: "2026-07-03"
confidence: "hypothesis"
---

# Démocratie

## Pourquoi ce document existe

Ce document définit la place de la démocratie dans le modèle.

Project Horizon ne cherche pas à supprimer la démocratie, mais à clarifier ce qui doit relever du vote et ce qui doit relever de contraintes scientifiques.

---

## Constat actuel

La démocratie représentative permet la légitimité populaire, mais elle est exposée au court-termisme, à la polarisation et à l'influence des intérêts organisés.

Elle peut aussi donner l'impression que tout est négociable, y compris les contraintes physiques.

---

## Objectifs

Le modèle démocratique doit :

- préserver les libertés fondamentales ;
- permettre aux citoyens de contrôler les institutions ;
- éviter que les faits scientifiques soient réécrits par intérêt politique ;
- organiser des choix de société réels ;
- rendre les arbitrages compréhensibles ;
- permettre la contestation et la révision.

---

## Proposition

Les citoyens choisissent entre des modèles validés scientifiquement.

Cette validation ne remplace pas la démocratie. Elle fixe le périmètre des trajectoires compatibles avec les contraintes établies.

Ils participent notamment aux arbitrages portant sur :

- la justice sociale ;
- le niveau de redistribution ;
- les priorités de confort ;
- les seuils de dignité du socle vital ;
- la répartition de l'effort entre groupes sociaux ;
- les limites acceptables à certaines consommations non essentielles.

### Mécanismes démocratiques retenus

Le modèle retient plusieurs mécanismes complémentaires :

- assemblées citoyennes de révision sur les priorités sociales ;
- droit de recours individuel et collectif contre les décisions fondées sur un modèle validé ;
- publication des hypothèses, incertitudes et alternatives rejetées ;
- contrôle parlementaire des usages d'urgence ;
- contre-propositions citoyennes ou territoriales lorsqu'elles restent compatibles avec les contraintes physiques.

Les plans sectoriels, par exemple sur l'énergie, le logement, la mobilité ou l'alimentation, doivent donc être traités comme des outils techniques soumis à arbitrage démocratique.

---

## Justification

On ne vote pas la gravité, le climat ou les limites biologiques.

Mais on peut légitimement voter sur la manière de répartir les efforts à l'intérieur de ces limites.

---

## Alternatives étudiées

### Démocratie directe généralisée

Peut renforcer l'implication citoyenne, mais risque de rendre les sujets complexes vulnérables à la simplification et à l'émotion immédiate.

### Démocratie représentative classique

Simple et connue, mais insuffisante face aux enjeux de long terme.

### Technocratie sans participation

Peut être efficace à court terme, mais risque de perdre la légitimité et d'encourager la contestation radicale.

---

## Critiques

- Les citoyens pourraient se sentir privés de pouvoir réel.
- Les modèles validés peuvent être présentés de manière biaisée.
- La participation citoyenne peut devenir symbolique.
- Les institutions peuvent être trop complexes pour être contrôlées.
- Les plans sectoriels peuvent transférer des coûts importants vers certains territoires ou professions.

---

## Questions ouvertes

- Place exacte du tirage au sort dans les assemblées citoyennes.
- Conditions d'usage des référendums.
- Format d'information publique permettant d'éviter la propagande institutionnelle.
- Procédure à suivre lorsqu'une majorité refuse une trajectoire scientifiquement robuste mais socialement contestée.

---

## Ce qui pourrait nous faire changer d'avis

Ce chapitre devrait être révisé si le modèle réduit trop la souveraineté citoyenne, si les citoyens ne comprennent plus les décisions, ou si la participation devient purement décorative.
